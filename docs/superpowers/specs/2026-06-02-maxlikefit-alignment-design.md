...
4. **numba 加速 core**：5 个高斯型 core（`_chi_square_core`/`_chi_square_full_core`/`_log_chi_square_core`/`_vdr_core`/`_odr_core`）用 `@nb.njit(cache=True, fastmath=True)`，写成**显式循环**（bayspec `_gstat_core` 风格）以便 njit；`_groth_core` 用 `Decimal`+`factorial` 无法 njit，保持纯 Python。njit core 只做数组运算；调用模型的 Python 闭包留在 wrapper 里（与 bayspec 一致）。
5. **统一 `**kwargs` 入口**：六个统计量 wrapper 签名统一为 `Name(**kwargs)`，kwargs 固定为 `{mo_func, params, x, y, x_err, y_err, w, up}`（不需要的键各自忽略，如 `chi^2` 忽略 `x_err`）。Pair 以关键字方式调用（仿 bayspec `_allowed_stats[stat](S=S, ...)`）。

## 2. `infer/statistic.py` 重构

- 每个统计量 `@staticmethod Name(**kwargs)`：从统一 kwargs 取 `mo_func, params, x, y, x_err, y_err, w, up`；计算 `my = mo_func(x, params)`；若 `my` 含非有限值，返回 `(inf, full(n, inf))`；否则委托给模块级 `_name_core(...)`，返回 `(stat, residual)`（仿 bayspec `Gstat`→`_gstat_core`）。
- `stat = ΣSᵢ`（即 −2·loglike；`groth` 为 −2·lnL）。`residual` 按决策 3 定义。

核心函数（前 5 个 `@nb.njit(cache=True, fastmath=True)` 显式循环；`_groth_core` 纯 Python）：
...
循环内逐点：按 `(y<my)` 选上/下误差 → 算 `Sᵢ` → `up` 处理（允许置 0、违反置 inf）→ 累加 `stat`、写 `residualᵢ = sign(y−my)·sqrt(Sᵢ if Sᵢ>0 else 0)`。公式逐字沿用现有 `Statistic`。`vdr`/`odr` 取 `k=params[0]`、`logv=params[2]`（线性模型约定）；`chi^2f` 取 `logv=params[-1]`。
...
- `stat_func`（cached）：`lambda **kw: _allowed_stats[kw.pop('stat')](**kw)[0]`，以关键字传 `mo_func/params/x/y/x_err/y_err/w/up`（非有限保护已在 wrapper 内）。
- `pseudo_residual_func`（cached）：同上取 `[1]`。
- `_stat_calculate` / `_pseudo_residual_calculate`：逐 unit 组装 kwargs（`mo_func=self.mo_func, params=self.pvalues, x=unit.x, y=unit.y, x_err=unit.xerr, y_err=unit.yerr, w=unit.weight, up=unit.up, stat=unit.stat`）并调用。
